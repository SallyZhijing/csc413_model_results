...
# Define a hybrid CNN+ViT model
class HybridCNNViT(nn.Module):
    def __init__(self, num_classes=2):
        super(HybridCNNViT, self).__init__()
        # Define a small CNN block
        self.cnn_block = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            # No pooling here to keep dimension 224x224
        ...
        # Change the head for binary classification
        self.vit.heads.head = nn.Linear(self.vit.heads.head.in_features, num_classes)

        # Freeze all but the last 3 layers and the head
        self.vit = freeze_all_but_last_3_layers_and_head(self.vit)

    def forward(self, x):
        # Pass through CNN block first
        x = self.cnn_block(x)
        # Then pass through ViT
        x = self.vit(x)
        return x
