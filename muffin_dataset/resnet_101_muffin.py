...
# We have only one dataset now: muffin_chihuahua
...
# Initialize model - pretrained ResNet101
...
# Save the model
model_path = f"best_model_resnet101_pretrained{True}_{dataset_name}.pth"
...
loss_results = {dataset_name: val_loss_history}
acc_results = {dataset_name: val_acc_history}

# Plot validation loss
plt.figure(figsize=(10, 5))
for name, loss_history in loss_results.items():
    plt.plot(range(len(loss_history)), loss_history, label=f'{name} Validation Loss')
plt.title('Validation Loss Comparison')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.savefig(f'validation_loss_comparison_resnet101_pretrained{True}_{dataset_name}.png')
plt.show()

# Plot validation accuracy
plt.figure(figsize=(10, 5))
for name, acc_history in acc_results.items():
    plt.plot(range(len(acc_history)), acc_history, label=f'{name} Validation Accuracy')
plt.title('Validation Accuracy Comparison')
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.legend()
plt.savefig(f'validation_accuracy_comparison_resnet101_pretrained{True}_{dataset_name}.png')
plt.show()
