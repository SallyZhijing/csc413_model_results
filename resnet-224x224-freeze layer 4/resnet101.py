...
def train_model(model, criterion, optimizer, dataloaders, dataset_sizes, num_epochs=10, run_label="50%"):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            # Aggregate all predictions and labels for the phase
            all_preds = np.concatenate(all_preds)
            all_labels = np.concatenate(all_labels)
            epoch_loss = running_loss / dataset_sizes[phase]
            epoch_accuracy, epoch_precision, epoch_f1 = compute_metrics(torch.tensor(all_preds), torch.tensor(all_labels))

            elapsed_time = time.time() - start_time

            print(f"{phase.capitalize()} Loss: {epoch_loss:.4f}, Acc: {epoch_accuracy:.4f}, Precision: {epoch_precision:.4f}, F1: {epoch_f1:.4f}, Time: {elapsed_time:.2f}s")
            ...
            epoch_results[f'{phase}_f1'] = epoch_f1
