...
# Choose two classes: cat (3) and dog (5)
class_to_include = [3, 5]
...
# train_dataset_full is 100%

def get_dataloaders(train_subset, val_dataset, batch_size=32):
    dataloaders = {
...
def freeze_all_but_last_3_layers_and_head(model):
    ...
    # The ViT layers are often in `model.blocks`
...
# Define a hybrid CNN+ViT model using a smaller pretrained ViT (vit_tiny_patch16_224)
class HybridCNNViT(nn.Module):
    def __init__(self, num_classes=2):
        super(HybridCNNViT, self).__init__()
        # A slightly more complex CNN block
        ...
        # Load smaller pretrained ViT from timm
        ...
        # Freeze all but the last 3 layers and head
        self.vit = freeze_all_but_last_3_layers_and_head(self.vit)

        # Add a dropout layer before the final classification
        self.dropout = nn.Dropout(p=0.3)

    def forward(self, x):
        # Pass through CNN block first
        x = self.cnn_block(x)  # Still 3x112x112
        # The vit_tiny_patch16_224 expects 3x224x224. We resized initially to 224x224,
        # but after maxpool we got 112x112. Let's upsample back to 224x224 for ViT input.
...
# Save results to CSV
csv_filename = "training_results_hybrid.csv"
fieldnames = [
    'run', 'epoch',
...
plt.legend()
plt.grid(True)
plt.savefig('accuracy_plot_hybrid.png')
plt.show()
